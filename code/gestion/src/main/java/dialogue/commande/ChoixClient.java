...
public class ChoixClient extends JDialog {
    ...
    public ChoixClient(Window parent, Connexion connexion) {
    	super(parent);
    ...
    private void initialize() {
		...
		panel.setBackground(new Color(0x00, 0x99, 0XCC));
    ...
    private JLabel getLblVeuillezChoisirDans() {
		if (lblVeuillezChoisirDans == null) {
		    lblVeuillezChoisirDans = new JLabel("Double-cliquez sur la ligne du client.");
	...
	private JTable getJTable() {
		if (jTable == null) {
			...
			jTable = new JTable(modele);
			jTable.addMouseListener(new MouseAdapter() {

				public void mousePressed(MouseEvent e) {
					if (e.getClickCount() == 2) {
						int ligne = jTable.getSelectedRow();
						client = modele.getClient(ligne);

						ChoixClient.this.dispose();
					}
				}
			});
			TableColumnModel colonnes = jTable.getColumnModel();
			TableColumn fidelite = colonnes.getColumn(3);
			fidelite.setCellRenderer(new BooleenRenderer());
			TableColumn dates = colonnes.getColumn(4);
    ...
    private JLabel getLab_recherche() {
		if (lab_recherche == null) {
		    ...
		    lab_recherche.setIcon(new ImageIcon(ChoixClient.class.getResource("/images/gestion/Search-48.png")));
		}
		...
		if (rechercher == null) {
		    ...
		    rechercher.addKeyListener(new KeyAdapter() {
		    	public void keyReleased(KeyEvent arg0) {
		    		controleClient.rechercher(rechercher.getText());
		    	}
		    });
