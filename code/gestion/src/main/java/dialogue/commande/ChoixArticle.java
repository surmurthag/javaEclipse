...
public class ChoixArticle extends JDialog {
	...
	private JTable getJTable() {
		if (jTable == null) {
			ModeleArticles modele = controleArticle.getModele();
			...
			jTable.addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					if (e.getClickCount() == 2) {
						...
						article = modele.getArticle(ligne);
						if (article.getQuantite() == 0) {
							JOptionPane.showMessageDialog(null,
									"Stock épuisé pour cet article", "Ajout",
									JOptionPane.WARNING_MESSAGE);
						} else {
							ChoixArticle.this.dispose();
						}
					}
				}
			});
		...
		if (rechercher == null) {
		    ...
		    rechercher.addKeyListener(new KeyAdapter() {
		    	public void keyReleased(KeyEvent arg0) {
					controleArticle.rechercher(rechercher.getText());
		    	}
		    });
